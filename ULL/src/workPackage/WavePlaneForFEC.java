...
public class WavePlaneForFEC {
	...
	public static void WavePlaneAlgorithm(Layer layer,int DemandNum,int WavelengthNum){
		...
		//按照波平面分配波长
		AssginmentForDemandBasedOnWavePlane(layer,WavePlanelist,DemandNum,WavelengthNum);
	 ...
	 * @param W_number
	 */
	public static void AssginmentForDemandBasedOnWavePlane(Layer layer,ArrayList<WavePlane> WavePlanelist,int DemandNum,int W_number){
    	...
    	while(KeyValue>0){   		 
    		...
    		while(iter2.hasNext()){
    			...
    			for(WavePlane currentWavePlane:WavePlanelist)
    			{
    				...
    				//	System.out.println("waveplane: "+WavelengthIndex);
    			...
    			for(WavePlane currentWavePlane:WavePlanelist)
    			{
    				currentWavePlane.OutputConstraintlinks();
    			}
    			*/
