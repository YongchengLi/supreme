...
public class DataEvacutation_CF {
    ...
    //  function.outputLinkRisk(phyLayer, rand);
    	...
    	if(key){
        	...
        	System.out .println("test: "+contentList.size());
        	test.setRisk_CF(phyLayer,contentList, damagedNodelist,rapidConetentList,Constant.r);
    	...
    	for(Content iCon:rapidConetentList)
		{
    		...
    		//if(!iCon.isSafeStatus())
    		{
        	    ...
        	    if(key)
        	    {
        	    	...
        	    	CF_RDE(phyLayer,iCon,dataCenterlist,avaDataCenterlist);
        	    }
        	    else
        	    {
   ...
   //   OutputContentSie(rapidConetentList);
       System.out.println("test: "+DeliveryedData);
		...
		for(Content iCon:contentList)
		{
			...
			for(int i=0;i<iCon.getBlocklist().size();i++)
			{
				...
				while(!status){
					...
					iDataCenter = rand.nextInt(Constant.maxiIndex-Constant.miniIndex)+Constant.miniIndex+1;
