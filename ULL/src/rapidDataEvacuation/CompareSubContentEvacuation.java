...
public class CompareSubContentEvacuation {

	
	//input file
	public static String ReadFile =  "24_01";
	...
	public static double NumLimit = 1000;
	...
	public static void main(String[] args) {
		...
		file_out_put result = new file_out_put();
    ...
    //     Constant.conNum = NumContent;
	//	   Constant.conNum = NumContent;//NumContent;
		   
		    String mode = Constant.mode2;
		    result.filewrite("Result/result_"+seed+".csv", "================="+ Constant.conNum+"-"+ Constant.maximumSize+"-"+mode+"===============");
		    //run the algorithm
			run(function,mode,result);
			System.out.println("totalMaxTime: "+totalMaxTime);
		
			long endTime =  System.currentTimeMillis();
				
			long calTime = endTime - startTime;
			double TotalcalTime = (double)calTime/1000;
			//System.out.println("TotalcalTime: "+TotalcalTime);
			totalMaxTime = totalMaxTime+TotalcalTime;
			result.filewrite("Result/result_"+seed+".csv", "EvacuatedContentNum: ,"+EvacuatedContentNum);
		    result.filewrite("Result/result_"+seed+".csv", "totalMaxTime: ,"+totalMaxTime);
	 ...
	 * @param function
	 */
	public static void run(Functions_Rapid_DataEvacuation function,String mode,file_out_put result){

		
		//Generate Physical Layer
    	Layer phyLayer=new Layer("physicallayer", 0, "");  
        phyLayer.readTopologywithDataCenter(ReadFileName);                                                                         
    	...
    	function.setLinkCapacity(phyLayer,rand);
    	//phyLayer.SetCostofAllLink();
    	//Search Two shortest routes for each node pair   
    	Functions_Search_Route PathSearch = new Functions_Search_Route();
     
    	PathSearch.KshortestPath(phyLayer, Constant.routeNumber);
    	
    	//phyLayer.OutputAllLink();
      	
        //set the linkCapacity
        
      
    	
  
    	//occur an earthquake
    	Earthquake earthquake = new Earthquake("earthquake",0, "occured earthquake");
    	function.InputEarthquake(earthquake,earX,earY,earStrength);
    	
    	
    	
    	
    	
    	//find the damaged datacenter
    	...
    	GenerateDataCenterList(phyLayer,rand,dataCenterlist);
    	
    //	GenerateConetentList(contentList,rand,dataCenterlist);
    	GenerateConetentListEqual(contentList,rand,dataCenterlist);
    	test.OutputDataCenterInformation(phyLayer);
   
 

    	test.setRisk(phyLayer, damagedNodelist,rapidConetentList);
    	...
    	if(mode =="MSCRDE")   	
    		ContentDivision(rapidConetentList);
    	
    	for(Content iCon:rapidConetentList)
		{
    		if(!iCon.isSafeStatus())
    		{
        		...
        		SelectAvaDataCenter(iCon,dataCenterlist,avaDataCenterlist);
        	    System.out.println(avaDataCenterlist.size());
        		if(mode =="NRDE")
        			Strategy_NearestDataCenter(phyLayer,iCon,avaDataCenterlist);
        		//	Strategy_SinglePath_SingleDestinationNode(phyLayer,iCon,avaDataCenterlist);
        		if(mode =="LDRDE")
        			Strategy_MultiPath_SingleDestinationNode(phyLayer,iCon,avaDataCenterlist);
        		if(mode =="MSCRDE")
        		//	Strategy_MultiPath_SubContent_SingleDestinationNode(phyLayer,iCon,avaDataCenterlist);	
        			TestStrategy_MultiPath_SubContent_SingleDestinationNode(phyLayer,iCon,avaDataCenterlist);	
        		//	TestStrategy_MultiReplica_SubContent_SingleDestinationNode(phyLayer,iCon,avaDataCenterlist);	
        		//   TestStrategy_MultiPath_SingleDestinationNode(phyLayer,iCon,avaDataCenterlist);	
    		}
		}
    	 OuputDataDistribution(phyLayer,mode,result);
    	
    	
	}
	public static void GenerateMultiReplicaConetentList()
	{
	...
	public static void GenerateConetentList(ArrayList<Content> contentList,Random rand,ArrayList<DataCenter> dataCenterlist){
		...
		for(int i = 0; i<Constant.conNum;i++)
		{
			...
			int size = rand.nextInt(Constant.maximumSize-Constant.minimumSize)+Constant.minimumSize;
		...
		for(Content iCon:contentList)
		{
			...
			while(!status){
				...
				if(dataCenterlist.get(iDataCenter).getEmptyStorage()>iCon.getSize()){
	...
	public static void GenerateSub(Content iCon,ArrayList<Content> contentList)
	{
		for(int i = 0; i<Constant.Num;i++)
		{
			Content iNewCon = new Content(iCon.getName()+"-"+iCon.getIndex()+"-"+i, iCon.getIndex(), iCon.getComments());
			System.out.println("Copy it:"+iCon.getIndex());
			iNewCon.setSize(iCon.getSize());
			iNewCon.setWeight(iCon.getWeight());
			
			iNewCon.setAssociatedDataCenter(iCon.getAssociatedDataCenter());
			iNewCon.setDataCenterList(iCon.getDataCenterList());
			
			for(DataCenter dataCenter:iNewCon.getDataCenterList())
			{
				int currentEmptyStorage =dataCenter.getEmptyStorage();

				dataCenter.getContentlist().add(iNewCon);
	
				dataCenter.setEmptyStorage(currentEmptyStorage-iNewCon.getSize());
				
			}
			
			
			
			contentList.add(iNewCon);
		}	
	}
	/**
	...
	public static void GenerateDataCenterList(Layer layer, Random rand,ArrayList<DataCenter> dataCenterlist)
	{
		...
		while(iterNode.hasNext()){
			...
			if(node.getIsDataCenter()){
				...
				int stroage = rand.nextInt(Constant.maximumStorage-Constant.minimumStorage)+Constant.minimumStorage;
	...
	public static void SortMaxtoMin(ArrayList<Content> contentList)
	{
	     ...
	     int index = 0;
	     ArrayList<Content> addContentList = new ArrayList<Content>();
	     for(Content con:contentList)
	     {
	        	
	    	 if(index<size)
	        	{
	        		GenerateSub(con,addContentList);
	        		index++;
	        	}
	        	else{
	        		break;
	     ...
	     for(Content con:addContentList)
	     {
	    	 contentList.add(con);
	...
	public static double Delay(LinearRoute route,int conSize,int linkCapacity)
	{
		...
		double transmission_delay = (double)conSize/linkCapacity*1000000;
...
*/	
	...
	public static void deliveryContentWithoutTrafficGromming(Layer layer,Task task)
	{
		...
		for(Link link:task.getRoute().getLinklist())
		{
		    ...
		    if(link.getResidualCapacity()-timeslot.getOccupyCapacity()>0&&link.getResidualCapacity()-timeslot.getOccupyCapacity()<link.getTimeCapacity().getMinimumResidualCapacity())
			...
			Link anotherlink = layer.findlink(link.getNodeB(), link.getNodeA());
			anotherlink.setLinkdelay(task.getTotalTime());
			if(anotherlink.getResidualCapacity()-task.getContent().getSize()>0&&anotherlink.getResidualCapacity()-task.getContent().getSize()<anotherlink.getTimeCapacity().getMinimumResidualCapacity())
